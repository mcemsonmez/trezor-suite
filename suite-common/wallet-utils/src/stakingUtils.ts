...
interface GetUnstakingPeriodInDays {
    withdrewAt?: number | null; // in seconds
    exitedAt?: number | null; // in seconds
}

export const getUnstakingPeriodInDays = (
    networkType: NetworkType | undefined,
    { withdrewAt, exitedAt }: GetUnstakingPeriodInDays = {},
) => {
    ...
    if (typeof withdrewAt !== 'number' || typeof exitedAt !== 'number') {
        return UNSTAKING_ETH_PERIOD;
    }

    const unstakingPeriodInSeconds = new BigNumber(withdrewAt).plus(exitedAt).toNumber();
