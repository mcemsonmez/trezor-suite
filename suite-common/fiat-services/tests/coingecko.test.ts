...
describe('findClosestTimestampValue', () => {
    ...
    test('marks graph month resolution as stale after twenty-four hours', () => {
        const now = Date.UTC(2026, 0, 1);

        expect(
            isGraphHistoricResolutionStale(asTimestamp(now - 24 * 60 * 60 * 1000), 'month', now),
        ).toBe(false);
        expect(
            isGraphHistoricResolutionStale(
                asTimestamp(now - 24 * 60 * 60 * 1000 - 1),
                'month',
                now,
            ),
        ).toBe(true);
