    ...
    async (dispatch: Dispatch, getState: GetState) => {
        ...
        const fetchSegment = async (
            ...
            const onProgress = (event: BlockchainAccountGraphProgress) => {
                ...
                console.log(
                    `[graphActions] progress for ${account.symbol}: +${event.progress.data.length} buckets, ${progressiveHistory.length} total`,
                );
            ...
            if (response?.success) {
                const totalTxs = response.payload.reduce(
                    (sum: number, entry: { txs: number }) => sum + entry.txs,
                    0,
                );
                console.log(
                    `[graphActions] balance history for ${account.symbol}: ${response.payload.length} buckets covering ${totalTxs} txs`,
                );
            } else {
                console.log(
                    `[graphActions] balance history for ${account.symbol}: failed`,
                    response,
                );
            }
